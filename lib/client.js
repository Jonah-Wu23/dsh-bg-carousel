window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		async function pickDirectoryOnHost() {
		...
		*/
		async function openDirOnHost(dir) {
		...
		function Panel() {
			...
			const pickDir = () => {
				...
				pickDirectoryOnHost().then((picked) => {
					...
					fetchJson("/settings", {
						method: "POST",
						body: JSON.stringify({ mediaDir: picked })
					}).then(() => refresh()).catch((e) => setError(String(e)));
				}).catch((e) => setError(String(e)));
			};
			const openDir = () => {
